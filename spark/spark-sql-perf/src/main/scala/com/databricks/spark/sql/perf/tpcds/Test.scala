...
object BenchSQLDS {
    def main(args: Array[String]) {
		if(args.length < 5){
			System.err.println("Usage: <inputPath> <outputPath> <scaleFactor> <iterations> <dsdgenDir>")
			System.exit(-1)
		}
		...
		val iterations = args(3)
		val dsdgenDir = args(4)
		
		// Tables in TPC-DS benchmark used by experiments.
		val tables = new TPCDSTables(sqlContext, dsdgenDir, scaleFactor)
		
		tables.genData(outputPath, "parquet", true, false, false, false)
